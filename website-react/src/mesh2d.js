...
class Mesh2 {
    constructor(source) {
        ...
        // string in format "x1,y1 x2,y2 x3,y3..."
            ...
            for (var j = 0; j < rawVertices.length; j += 2) {
                this.vertices.push(new Vec2(rawVertices[j] + "," + rawVertices[j + 1]));
            }
        }
    }
    ...
    add(other) {
        if (this.degree() !== other.degree()) {
            throw new Error("cannot add meshes of different degrees");
        }
        var vertices = []
    ...
    sub(other) {
        if (this.degree() !== other.degree()) {
            throw new Error("cannot subtract meshes of different degrees");
        }
        var vertices = []
        for (var i = 0; i < this.degree(); i++) {
    ...
    morphTo(target, initialMesh, duration, listener) {
        if (this.degree() === 0) {
            throw new Error("cannot morph mesh of degree 0");
        } else if(this.degree() !== target.degree()) {
            throw new Error("cannot morph meshes of different degrees");
        }
